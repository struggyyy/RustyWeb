...
export default function PublicHeader() {
  ...
  const { user, logOut, isAdmin, updateUserProfile, previousPath } = useAuth(); // Added previousPath
  ...
  const handleBack = () => {
    ...
    setTimeout(() => {
      // 1. Check if we have a tracked previous path from AuthContext (reliable for SPA nav)
      if (previousPath) {
        ...
        // If previous page was protected and we are now logged out -> Home
      ...
      // 2. Fallback to referrer check (for first loads or external entries)
      else if (
        document.referrer &&
        document.referrer.startsWith(window.location.origin)
      ) {
        // Same check for referrer
            ...
            <div className="fixed top-16 left-0 right-0 mx-auto w-72 min-[400px]:absolute min-[400px]:top-14 min-[400px]:right-0 min-[400px]:left-auto min-[400px]:mx-0 bg-neutral-50/60 backdrop-blur-2xl border border-white/60 dark:border-neutral-700/60 shadow-2xl dark:shadow-[0_0_30px_rgba(255,255,255,0.15)] rounded-2xl p-2 animate-in fade-in slide-in-from-top-2 duration-200 origin-top min-[400px]:origin-top-right z-40">
              ...
              <div className="relative z-10 space-y-1">
                ...
                >
                  ...
                  {isAdmin ? t("admin.managementTitle") : t("dashboard.title")}
                </Link>
